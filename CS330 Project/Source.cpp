...
namespace
{
	const char* const WINDOW_TITLE = "Darren Sheftic - CS330 Project"; // Macro for window title
...
void main()
{
	...
	{
		// Set object color to (1.0, 1.0, 1.0) ignoring lighting calculations
		phong1 = vec3(1.0, 1.0, 1.0);
		phong2 = vec3(1.0, 1.0, 1.0);
	}
...
int main(int argc, char* argv[])
{
	...
	if (!UCreateShaderProgram(vertexShaderSource, fragmentShaderSource, gProgramId))
		return EXIT_FAILURE;
	// Load texture
	const char* texFilename = "DeskTexture.jpg";
	if (!UCreateTexture(texFilename, gDesk))
	{
		cout << "Failed to load texture " << texFilename << endl;
		return EXIT_FAILURE;
	}
	texFilename = "LampTexture.jpg";
	if (!UCreateTexture(texFilename, gLamp))
	{
		cout << "Failed to load texture " << texFilename << endl;
		return EXIT_FAILURE;
	}
	texFilename = "LaptopTexture.jpg";
	if (!UCreateTexture(texFilename, gLaptop))
	{
		cout << "Failed to load texture " << texFilename << endl;
		return EXIT_FAILURE;
	}
	texFilename = "CupTexture.jpg";
	if (!UCreateTexture(texFilename, gCup))
	{
		cout << "Failed to load texture " << texFilename << endl;
		return EXIT_FAILURE;
	}
	texFilename = "CupInsideTexture.jpg";
	if (!UCreateTexture(texFilename, gCupInside))
	{
		cout << "Failed to load texture " << texFilename << endl;
		return EXIT_FAILURE;
	}
	...
	glBindTexture(GL_TEXTURE_2D, gLamp);
	glActiveTexture(GL_TEXTURE2);
	glBindTexture(GL_TEXTURE_2D, gLaptop);
	glActiveTexture(GL_TEXTURE3);
	glBindTexture(GL_TEXTURE_2D, gCup);
	glActiveTexture(GL_TEXTURE4);
	glBindTexture(GL_TEXTURE_2D, gCupInside);
...
void URender()
{
	...
	glClearColor(0.29f, 0.145f, 0.26f, 1.0f);
	...
	glUniform3f(light1ColLoc, 0.85f, 0.85f, 0.9f);
	glUniform3f(light1PosLoc, -4.99f, 6.0, -0.15f);
	glUniform3f(light2ColLoc, 0.29f, 0.145f, 0.26f);
	glUniform3f(light2PosLoc, 0.0f, 1.0f, 0.0f);
	//set specular intensity
	glUniform1f(specInt1Loc, 0.50f);
	glUniform1f(specInt2Loc, 0.01f);
	//set specular highlight size
	glUniform1f(highlghtSz1Loc, 1.0f);
	glUniform1f(highlghtSz2Loc, 0.1f);
	...
	// 1. Scales the object
	glm::mat4 pModelScale = glm::scale(glm::vec3(1.0f, 1.0f, 1.0f));
	// 2. Rotate the object
	glm::mat4 pModelRotation = glm::rotate(0.0f, glm::vec3(1.0, 1.0f, 1.0f));
	// 3. Position the object
	glm::mat4 pModelTranslation = glm::translate(glm::vec3(0.0f, 0.0f, 0.0f));
	// Model matrix: transformations are applied right-to-left order
	glm::mat4 pModel = pModelTranslation * pModelRotation * pModelScale;
	...
	// Model matrix: transformations are applied right-to-left order
	model = pModel * translation * rotation * scale;
	glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(model));

	glUniform1i(glGetUniformLocation(gProgramId, "uTexture"), 0); // Texture unit 0
	...
	// 1. Scales the object
	scale = glm::scale(glm::vec3(-6.0f, 0.2, 4.0f));
	// 2. Rotate the object
	rotation = glm::rotate(0.0f, glm::vec3(1.0, 1.0f, 1.0f));
	// 3. Position the object
	translation = glm::translate(glm::vec3(0.0f, 0.1f, 1.0f));
	// Model matrix: transformations are applied right-to-left order
	model = pModel * translation * rotation * scale;
	glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(model));

	//glProgramUniform4f(gProgramId, objectColorLoc, 0.7f, 0.7f, 0.7f, 0.8f);
	// bind textures on corresponding texture units
	glUniform1i(glGetUniformLocation(gProgramId, "uTexture"), 2); // Texture unit 2
	...
	rotation = glm::rotate(0.0f, glm::vec3(1.0, 1.0f, 1.0f));
	// 3. Position the object
	translation = glm::translate(glm::vec3(-4.8f, 0.0f, -1.0f));
	// Model matrix: transformations are applied right-to-left order
	model = pModel * translation * rotation * scale;
	glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(model));

	glUniform1i(glGetUniformLocation(gProgramId, "uTexture"), 1); // Texture unit 1
	...
	// 1. Scales the object
	scale = glm::scale(glm::vec3(0.2f, 5.0f, 0.2f));
	// 2. Rotate the object
	rotation = glm::rotate(glm::radians(-350.0f), glm::vec3(1.0, 1.0f, 1.0f));
	// 3. Position the object
	translation = glm::translate(glm::vec3(-4.8f, 0.0f, -1.0f));
	// Model matrix: transformations are applied right-to-left order
	model = pModel * translation * rotation * scale;
	...
	// 1. Scales the object
	scale = glm::scale(glm::vec3(0.75f, 1.0f, 1.0f));
	// 2. Rotate the object
	rotation = glm::rotate(glm::radians(180.0f), glm::vec3(1.0, 0.0f, 0.0f));
	// 3. Position the object
	translation = glm::translate(glm::vec3(3.5f, 1.001f, -2.5f));
	// Model matrix: transformations are applied right-to-left order
	model = pModel * translation * rotation * scale;
	glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(model));

	glUniform1i(glGetUniformLocation(gProgramId, "uTexture"), 4); // Texture unit 4
	...
	rotation = glm::rotate(glm::radians(180.0f), glm::vec3(1.0, 0.0f, 0.0f));
	// 3. Position the object
	translation = glm::translate(glm::vec3(3.5f, 1.0f, -2.5f));
	// Model matrix: transformations are applied right-to-left order
	model = pModel * translation * rotation * scale;
	glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(model));

	glUniform1i(glGetUniformLocation(gProgramId, "uTexture"), 3); // Texture unit 3
	...
	// 1. Scales the object
	scale = glm::scale(glm::vec3(1.25f, 1.25f, 1.0f));
	// 2. Rotate the object
	rotation = glm::rotate(45.0f, glm::vec3(1.0, 1.0f, 1.0f));
	// 3. Position the object
	translation = glm::translate(glm::vec3(-5.0f, 6.0, -0.15f));
	// Model matrix: transformations are applied right-to-left order
	model = pModel * translation * rotation * scale;
	glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(model));
	...
	rotation = glm::rotate(45.0f, glm::vec3(1.0, 1.0f, 1.0f));
	// 3. Position the object
	translation = glm::translate(glm::vec3(-4.99f, 6.0, -0.15f));
	// Model matrix: transformations are applied right-to-left order
	model = pModel * translation * rotation * scale;
	glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(model));
